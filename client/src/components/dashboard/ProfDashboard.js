...
function ProfDashboard(props) {
  ...
  function createExam() {
    ...
    if (exam_link === "") {
      setErrorText(" ⚠️Exam Link cannot be empty ⚠️");
      return;
    }
    if (!isUrl(exam_link)) {
      setErrorText("⚠️ Exam Link must be a valid url ⚠️");
      return;
    }
    if (duration === 0) {
      setErrorText(" ⚠️ Duration cannot be 0 ⚠️ ");
      return;
    }
    if (exam_code === "") {
      setErrorText("⚠️ Click Generate exam code 💻  to get an exam code first ⚠️");
      return;
    }
    var current_date_time = new Date();
    if (date_time_start < current_date_time) {
      setErrorText("⚠️ Please select a date 📅 and time  ⌚ of the future ⚠️");
      return;
    }
